...
const META_AUTH_URL_FALLBACK = 'https://meta-auth.clientesneurolinks.com/meta-auth';
const encodeMetaAuthPayload = (data: Record<string, any>): string => Buffer.from(JSON.stringify(data)).toString('base64url');
const signMetaAuthPayload = (payload: string, secret: string): string => createHmac('sha256', secret).update(payload).digest('base64url');
const getRequestBaseUrl = (req: any): string => {
    const envUrl = (process.env.PROJECT_URL || process.env.PUBLIC_BACKOFFICE_URL || '').trim();
    if (envUrl) return envUrl.replace(/\/$/, '');
    const proto = String(req.headers?.['x-forwarded-proto'] || req.protocol || 'https').split(',')[0].trim();
    const host = String(req.headers?.['x-forwarded-host'] || req.get?.('host') || req.headers?.host || '').split(',')[0].trim();
...
export const resolveServiceId = (req: any): string => {
...
export const registerBackofficeRoutes = (app: any) => {
    ...
    app.get('/api/backoffice/whatsapp/config', backofficeAuth, async (req: any, res: any) => {
        ...
        if (metaAuthSecret && requestBaseUrl) {
            metaAuthPayload = encodeMetaAuthPayload({
                projectId,
                serviceId,
                callbackUri,
                ...
                metaAppId: appId,
                configId,
                projectUrl: requestBaseUrl,
                exp: Math.floor(Date.now() / 1000) + 600
            });
            metaAuthSig = signMetaAuthPayload(metaAuthPayload, metaAuthSecret);
        }
    ...
    app.get('/api/backoffice/whatsapp/onboard-callback', async (req: any, res: any) => {
        ...
        try {
            ...
            const dashboardUrl = `${getRequestBaseUrl(req) || process.env.PROJECT_URL || ""}/dashboard.html?metaStatus=success`;
        ...
        try {
            ...
            const attemptCreateMetaWabaGroup = async (groupName: string, groupContacts: any[], metaConfig: any) => {
                ...
                if (groupContacts.length > 0) {
                    ...
                    const inviteMessage = `Â¡Hola! Te invitamos a unirte al grupo oficial de reportes de Neurolinks *"${groupName}"*. Para unirte, haz clic en el siguiente enlace de invitaciÃ³n: ${inviteLink}`;
                ...
                if (metaConfig?.whatsappToken && metaConfig?.whatsappNumberId) {
                    try {
                        ...
                        if (newContacts.length > 0) {
                            ...
                            const inviteMessage = `Â¡Hola! Te invitamos a unirte al grupo oficial de reportes de Neurolinks *"${name}"*. Para unirte, haz clic en el siguiente enlace de invitaciÃ³n: ${inviteLink}`;

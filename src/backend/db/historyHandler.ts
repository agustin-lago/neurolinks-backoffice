...
export class HistoryHandler {
    static readonly PROJECT_IDENTIFIER = process.env.PROJECT_ID || process.env.RAILWAY_PROJECT_ID || BACKOFFICE_MASTER_PROJECT_ID;
    static readonly PROJECT_ID = process.env.PROJECT_ID || process.env.RAILWAY_PROJECT_ID || BACKOFFICE_MASTER_PROJECT_ID;
    ...
    static async resolveTenantIdByProjectId(projectId: string): Promise<{ tenantId: string | null, resolved: boolean, globalScope: boolean }> {
        ...
        if (projectId === BACKOFFICE_MASTER_PROJECT_ID || LEGACY_MASTER_PROJECT_IDS.has(projectId) || projectId === 'neurolinks-control') {
    ...
    */
    ...
    private static async bootstrapConfig() {
        try {
            ...
            if (!apiKeySetting) {
                const uniqueKey = `sk_neurolinks_${crypto.randomBytes(16).toString('hex')}`;

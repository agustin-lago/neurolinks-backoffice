...
const mediaCache = {
    ...
    init() {
        return new Promise((resolve, reject) => {
            ...
            const request = indexedDB.open('NeurolinksMediaCache', 1);
...
function launchMetaOnboarding() {
    ...
    fetch(`/api/backoffice/whatsapp/config?token=${activeToken}&serviceId=${sId}&projectId=${pId}`)
        ...
        .then(data => {
            ...
            // Abrir Meta Auth con parámetros de redirección dinámica
            if (!data.metaAuthPayload || !data.metaAuthSig) {
                showToast('⚠️ Falta configurar META_AUTH_SHARED_SECRET', 'error');
                return;
            }
            const url = new URL(data.metaAuthUrl || 'https://meta-auth.clientesneurolinks.com/meta-auth');
            url.searchParams.append('payload', data.metaAuthPayload);
            url.searchParams.append('sig', data.metaAuthSig);
            ...
            url.searchParams.append('redirectUri', data.callbackUri || `${currentOrigin}/api/backoffice/whatsapp/onboard-callback?serviceId=${sId}`);
